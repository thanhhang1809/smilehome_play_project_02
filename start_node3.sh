...
## For dev env only!
## Start application in cluster mode: 3rd node (roles = [Role1, Role2, Role3])
...
sbt -Dconfig.file=conf/application-cluster.conf -Dlogger.file=conf/logback-dev.xml \
	-Dhttp.port=9003 -Dhttps.port=0 -Dthrift.port=0 -Dthrift.ssl_port=0 -Dgrpc.port=0 -Dgrpc.ssl_port=0 \
	...
	-D$CONF_PREFIX.akka.remote.netty.tcp.hostname=127.0.0.1 -D$CONF_PREFIX.akka.remote.netty.tcp.port=2553 \
	-Dakka.remote.netty.tcp.hostname=127.0.0.1 -Dakka.remote.netty.tcp.port=9053 \
	...
	-D$CONF_PREFIX.akka.cluster.roles.2=Role2 -Dakka.cluster.roles.2=Role2 \
	-D$CONF_PREFIX.akka.cluster.roles.3=Role3 -Dakka.cluster.roles.3=Role3 \
